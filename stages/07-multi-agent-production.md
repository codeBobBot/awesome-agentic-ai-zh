...
8. [**stablyai/orca**](https://github.com/stablyai/orca)（★ 63k+、MIT）— 目前規模最大的「**多 agent 同時跑**」桌面環境：一個 prompt 可以同時發給多個 agent，每個跑在自己的 git worktree 裡，跑完並排比較、挑一個 merge。它把 Codex / Claude Code / OpenCode / Pi 當成可替換的後端，所以真正的看點不是它支援誰，而是**當五個 agent 同時在改同一份 repo，介面要解決哪些問題**：隔離、比較、審查，以及「agent 做完了怎麼通知你」。附 iOS app，可以在手機上盯進度。
...
| | [AMAP-ML/LongHorizon-Harness](https://github.com/AMAP-ML/LongHorizon-Harness) | ⭐⭐⭐ | 想看「驗證那一格」在真實專案裡長什麼樣 | 把長任務拆成 Manager / Executor / **Auditor** 三個角色，Executor 每輪用新 context，Auditor 獨立檢查後才寫進持久 state——就是上面那張圖裡「檢查對不對」那一格的實作。包在 Claude Code / Codex 外層，不用自己重寫 agent loop。**很新**：2026-08-04 建立、2 位 contributor，還沒有長期維護紀錄。★ 1.5k+、MIT |
